...
def generate_figure2D(X, Y, label_list, pca_components=[0, 1], samples_per_class=1000):
    ...
    for j in range(n_classes):
        ...
        fig.add_trace(go.Scatter(x=x_coord, y=y_coord, name=label_list[j],mode='markers',marker={'size': size, 'symbol':'circle', 'color':colors[j]}), row=1, col=1)  #text=classes[j], 
   ...
   # )
    ...
    fig.update_layout(
        title="2D space (PCA)",
        xaxis_title=components_dict[pca_components[0]] + " principal component (x)",
        yaxis_title=components_dict[pca_components[1]] + " principal component (y)",
...
def generate_figure_training(epochs, val, loss):
    ...
    figure_training.add_trace(go.Scatter(x=epochs, y=val, name='val',mode='markers',marker={'size': size, 'symbol':'circle', 'color':colors[0]}), row=1, col=1)  #text=classes[j], 
    figure_training.add_trace(go.Scatter(x=epochs, y=loss, name='loss',mode='markers',marker={'size': size, 'symbol':'circle', 'color':colors[1]}), row=2, col=1)  #text=classes[j], 
