...
from dcase_models.util.events import contiguous_regions, event_roll_to_event_list, tag_probabilities_to_tag_list
...
from dcase_models.util.files import duplicate_folder_structure, list_wav_files, list_all_files
...
def test_sed():
    ...
    seg_metrics = SegmentBasedMetrics(
        label_list, time_resolution=time_resolution
    ) 
    event_pred = event_roll_to_event_list(
            event_roll, label_list, time_resolution)
...
def test_evaluate_metrics():
    ...
    results = evaluate_metrics(toy_model, ([X_val], [Y_val]), metrics, label_list=label_list)
