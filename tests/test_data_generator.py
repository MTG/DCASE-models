...
def test_init():
    ...
    data_generator = DataGenerator(dataset, feature_extractor, folds=['all'], shuffle=False)
...
def test_get_data_batch():
    ...
    # Train=False
    data_generator = DataGenerator(
        dataset, feature_extractor, folds=['all'], batch_size=2, shuffle=False, train=False)
    # check length
    assert len(data_generator) == 2
...
def test_convert_features_path_to_audio_path():
    ...
    audio_file = data_generator.convert_features_path_to_audio_path(features_file, features_path)
    ...
    features_file = os.path.join(features_path, 'test.npy')
    audio_file = data_generator.convert_features_path_to_audio_path(features_file, features_path, sr=sr)
    assert audio_file == os.path.join(audio_path, 'test.wav')
...
def test_convert_audio_path_to_features_path():
    ...
    features_file = data_generator.convert_audio_path_to_features_path(audio_file, features_path)
    assert features_file == os.path.join(features_path,  'test.npy') 

    # subfolder='original'
    features_file = data_generator.convert_audio_path_to_features_path(audio_file, features_path, subfolder='original')
