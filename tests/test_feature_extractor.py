...
def test_load_audio():
    ...
    audio = feature_extractor.load_audio(audio_file, mono=True, change_sampling_rate=True)
...
def test_pad_audio():
    ...
    assert len(audio_pad) == feature_extractor.sequence_frames*feature_extractor.audio_hop + feature_extractor.audio_win
    ...
    audio = np.ones(n_samples)
    audio_pad = feature_extractor.pad_audio(audio)
    assert len(audio_pad) == (feature_extractor.sequence_frames + feature_extractor.sequence_hop)*feature_extractor.audio_hop + feature_extractor.audio_win
    ...
    audio = np.ones(n_samples)
    audio_pad = feature_extractor.pad_audio(audio)
    assert len(audio_pad) == (2*feature_extractor.sequence_frames)*feature_extractor.audio_hop + feature_extractor.audio_win
...
def test_convert_to_sequences():
    ...
    feature_extractor = FeatureExtractor(sequence_hop_time=-1, sequence_time=-1, pad_mode='constant')
    frames = feature_extractor.convert_to_sequences(audio_rep) # (1, 74, 2)
    ...
    audio_pad = feature_extractor.pad_audio(audio)
    stft = librosa.core.stft(audio_pad, n_fft=1024,
                             hop_length=feature_extractor.audio_hop,
                             win_length=feature_extractor.audio_win, center=False)
