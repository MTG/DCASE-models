...
def get_fold_val(fold_test, fold_list):
    ...
    fold_val_ix = (fold_test_ix+1) % N_folds #sum(divmod(fold_test_ix+1,N_folds))
    fold_val = fold_list[fold_val_ix]
    return fold_val

def get_data_train(folds_data,fold_test,evaluation_mode,scaler=None,upsampling=True):
    ...
    if type(folds_data[fold_list[0]]['Y']) == list:
        for fold in fold_list:
            ...
            #folds_data[fold]['X'] = np.concatenate(folds_data[fold]['X'], axis=0)
            #folds_data[fold]['Y'] = np.concatenate(folds_data[fold]['Y'], axis=0)
        ...
        print(folds_data_numpy[fold]['X'].shape, folds_data_numpy[fold]['Y'].shape)
    else:
    ...
    if evaluation_mode == "train_validate_test":
        ...
        if N_folds > 2:    
            ...
            for fold_train in folds_train:   
                ...
                print(folds_data_numpy[fold_train]['X'].shape, folds_data_numpy[fold_train]['Y'].shape)
...
def get_data_train_list(folds_data,fold_test,evaluation_mode,scaler=None,upsampling=True):
    fold_list = list(folds_data.keys())
...
#from ..model.model import get_features
def get_data_test(model,folds_data_test,fold_test,folds_files,scaler):
    #print(len(folds_data_test[fold_test]['X']), len(folds_data_test[fold_test]['Y']), len(folds_files[fold_test]))
    ...
    for fold_train in folds_train:
        for file in range(len(folds_data_test[fold_train]['X'])):
            ...
            if len(X) <= 1:
                continue
            ix = int(len(X)/2)
            X = np.expand_dims(folds_data_test[fold_train]['X'][file][ix],axis=0)
            #print(ix,X.shape)
            X_train.append(X)
            Y = np.expand_dims(folds_data_test[fold_train]['Y'][file],axis=0)
            #print(Y.shape)
            Y_train.append(Y)
